...
vCoinWS.onReceiveDataEvent(async (place, score) => {
    ...
    setTerminalTitle("VCoinY " + getVersion() + " (id" + USER_ID.toString() + ") > " + formatScore(vCoinWS.tick, true) + " cps > " + "top " + place + " > " + formatScore(score, true) + " coins.");
    if (place > 0 && !rl.isQst) {
        if (transferPercent) {
            ...
            for (var i = 0; i < autoBuyItems.length; i++) {
                if (miner.hasMoney(autoBuyItems[i])) {
                    try {
                        ...
                        con("Новая скорость: " + formatScore(result.tick, true) + " коинов / сек.");
                        try {
                            await infLog(template);
                        } catch (e) {}
                    } catch (e) {
        ...
        if (!hidejunk)
            con("Место в топе: " + place + "\tКоличество коинов: " + formatScore(score, true) + "\tСкорость: " + formatScore(vCoinWS.tick, true) + " коинов / сек.", "yellow");
    }
});
...
vCoinWS.onBrokenEvent(_ => {
    ...
    setTerminalTitle("VCoinY " + getVersion() + " (id" + USER_ID.toString() + ") > " + "BROKEN");
...
vCoinWS.onAlreadyConnected(_ => {
    ...
    setTerminalTitle("VCoinY " + getVersion() + " (id" + USER_ID.toString() + ") > " + "ALREADY_CONNECTED");
    if (autobeep)
        beep();
...
function forceRestart(t, force) {
    ...
    if (boosterTTL)
        clearInterval(boosterTTL);
    setTerminalTitle("VCoinY " + getVersion() + " (id" + USER_ID.toString() + ") > " + "RESTARTING");
    if (xRestart || force)
        startBooster(t);
}
...
rl.on('line', async (line) => {
    ...
    switch (line.trim().toLowerCase()) {
        ...
        case 'info':
            ...
            con("Текущая скорость: " + formatScore(vCoinWS.tick, true) + " коинов / сек.\n");
            break;
        case 'color':
        ...
        case "run":
            if (vCoinWS.connected)
                return con("VCoinY уже запущен и работает!");
            xRestart = true;
            startBooster();
            break;
        case 'b':
